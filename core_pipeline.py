...
def post_request(url, headers, query):
    """Sends a POST request to the given URL with the provided query and headers."""
...
def fetch_data(exchange, offset, limit, log_file_path):
    ...
    try:
        response = requests.post(BASE_URL, headers=HEADERS, json={"query": QUERY_ALL_COMPANIES, "variables": variables})
...
def run_pipeline_id(Log_File, exchanges_file, fetch_step, base_path):
    print(f"📂 Running pipeline with log file: {Log_File}")
    print(f"📂 Using base path: {base_path}")
    print(f"🔄 Step size: {fetch_step}")
